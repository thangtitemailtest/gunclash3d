...
<div id="wrapper">
    ...
    <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
        ...
        <a class="sidebar-brand d-flex align-items-center justify-content-center" href="{{route('get-index')}}">
            ...
            <div class="sidebar-brand-text mx-3">Gun Clash 3D</div>
        </a>
            ...
            <div id="collapsePages13" class="collapse" aria-labelledby="headingPages" data-parent="">
                <div class="bg-white py-2 collapse-inner rounded">
                    ...
                    @if(in_array('report/sotiencuausertheolevel',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-sotiencuausertheolevel')}}">Số tiền của user
                            theo<br>các
                            level</a>
                    @endif

                    @if(in_array('report/levelusermuainapp',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-levelusermuainapp')}}">Level user mua inapp<br>nhiều
                            nhất</a>
                    @endif

                    @if(in_array('report/locsonguoichoipass',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-locsonguoichoipass')}}">Lọc số người chơi win
                            level</a>
                    @endif

                    @if(in_array('report/locsonguoichoilose',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-locsonguoichoilose')}}">Lọc số người chơi lose
                            level</a>
                    @endif

                    @if(in_array('report/locsonguoichoistart',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-locsonguoichoistart')}}">Lọc số người chơi play
                            level</a>
                    @endif

                    @if(in_array('report/locsotiennguoichoipass',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-locsotiennguoichoipass')}}">Lọc số tiền trung
                            bình<br>của
                            user chơi win level</a>
                    @endif

                    @if(in_array('report/locsotiennguoichoilose',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-locsotiennguoichoilose')}}">Lọc số tiền trung
                            bình<br>của
                            user chơi lose level</a>
                    @endif
                    {{--<a class="collapse-item" href="{{route('get-locsonguoiupgradebulletlevel')}}">Lọc số người upgrade bullet<br>level</a>--}}
                    {{--<a class="collapse-item" href="{{route('get-locsonguoiupgradebulletpower')}}">Lọc số người upgrade bullet<br>power</a>--}}

                    @if(in_array('report/quocgiacosologinnhieunhat',$permission) || $permission[0] == 'admin')
                        <a class="collapse-item" href="{{route('get-quocgiacosologinnhieunhat')}}">5 quốc gia có số
                            login<br>nhiều nhất</a>
                    @endif
                    {{--<a class="collapse-item" href="{{route('get-locsonguoichoigamelandau')}}">Lọc số người chơi game lần<br>đầu/tổng số người chơi game</a>--}}
